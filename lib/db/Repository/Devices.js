...
module.exports = (db)=> {
    ...
    const deviceRepository = {
        ...
        getBBs: async function() {
            let devices = [];
            let result = await db.devices.findAll({
                attributes: ['idDevice', 'auth_device', 'label'],
                include: [
                    {
        ...
        getDevicesLastData: async function() {
            ...
            for(var i = 0; i < devicesData[0].length; i++) {
                ...
                let query = "select devices.idDevice as idDevice, " +
                    ...
                    "IFNULL(peripheral_gps_data.orientation_plain, 0) as Direction, " +
                    ...
                    "(select max(alarm.id) from alarm where devices.idDevice = alarm.device and alarm.createdAt > CONVERT_TZ( NOW(), @@session.time_zone, '+00:00' ) - INTERVAL 5 MINUTE) " +
        ...
        saveSemovLog: function(json_data) {
            try {
                let insertQuery = 'insert into semov_log(data_json, created_at, updated_at) values(\'' + json_data + '\', now(), now())';
