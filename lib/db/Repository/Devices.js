...
module.exports = (db)=> {
    ...
    const deviceRepository = {
        ...
        getDevicesLastData: async function() {
            let query = "select ' ' as Provider, " +
                "' ' as IDCompany, " +
                "users.company_name as CompanyName, " +
                "vehicle.route as RouteName, " +
                "CONVERT(datetime, SWITCHOFFSET(CONVERT(datetimeoffset, peripheral_gps_data.createdAt), DATENAME(TzOffset, SYSDATETIMEOFFSET()))) AS local_date" +
                "DATE_FORMAT(local_date, '%d-%m-%Y') as Date, " +
                "DATE_FORMAT(local_date, '%H:%i:%s') as Time, " +
                "vehicle.vin as VIN, " +
                "vehicle.name as EconomicNumber, " +
                "vehicle.plate_number as VehiclePlate, devices.auth_device as IMEI, peripheral_gps_data.lat as Latitude, " +
                ...
                "if(alarm.id is not null, 'true', 'false') as PanicButton, camera.url_camera as UrlCamera " +
                "from devices " +
